...
switch($c) {
	case 'index':
		$config=$db->get_One('select * from `web_config` where id=1');
	...
	case 'save':		
		...
		$infos['tag']=trim($_POST['tag']);
		$infos['meta']=trim($_POST['meta']);
		$infos['tel']=trim($_POST['tel']);
		$infos['qq']=trim($_POST['qq']);
		$infos['icp']=trim($_POST['icp']);
		$infos['police']=trim($_POST['police']);
		$infos['kf']=trim($_POST['kf']);
		$infos['tongji']=trim($_POST['tongji']);
		$infos['join']=intval($_POST['join']);
		$infos['agreement']=htmlspecialchars($_POST['agreement']);
		if(empty($_POST['logo'])){
		...
		$db->update('web_config',$infos,'id=1');
