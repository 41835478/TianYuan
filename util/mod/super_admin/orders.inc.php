...
switch($c){
	case 'fahuo':
		...
		$info=$db->get_One('select * from `web_express_info` where oid='.$oid.' and gid='.$gid);
		...
		include T('super_admin','order_fahuo');
		break;
	case 'fhsave':
		$infos=array();
		...
		showmessage('快递信息保存成功！','/?m=admin&a=orders');
		break;
	case 'list':
		...
		if(!empty($oids)){
			$orders=$db->getAll("select * from web_order where id in ($oids) and status>0","id");
		}
		$pagestr=pages_y($nums['num'], $page, $offset);	
		include T('super_admin','orders_list');
		break;
	case 'del':
