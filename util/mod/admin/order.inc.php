<?php
$acts=array('index'=>true,'del'=>true,'neworder'=>true,'kd'=>true); 
$c=isset($_REQUEST['c'])?trim($_REQUEST['c']):'neworder';
if(!isset($acts[$c])){
	$c='neworder';
}
switch($c){
	...
	case 'neworder':
		...
		$offset=5;
		$start=($page-1)*$offset;
		$total=$db->get_one('select count(*) as num from `web_order` where userid='.$_userid.'  and status>0');
		...
		$num_yfk=$db->get_one('select count(*) as num from `web_order` where userid='.$_userid.'  and status=2');//退款
		if($_GET['org']=='dfk'){//待付款
			$nums=$db->get_one('select count(*) as num from `web_order` where userid='.$_userid.'  and status=1');
			$order=$db->getAll('select * from `web_order` where userid='.$_userid.' and status=1 order by updatetime desc,addtime desc limit '.$start.','.$offset,"id");
			...
			$nums=$db->get_one('select count(*) as num from `web_order` where userid='.$_userid.'  and status=2');
			$order=$db->getAll('select * from `web_order` where userid='.$_userid.' and status=2 order by updatetime desc,addtime desc limit '.$start.','.$offset,"id");
		}elseif($_GET['org']=='yfh'){
			$nums=$db->get_one('select count(*) as num from `web_order` where userid='.$_userid.' and status=3');
			$order=$db->getAll('select *from `web_order` where userid='.$_userid.' and
			status=3 order by updatetime desc,addtime desc limit '.$start.','.$offset,"id");
		}elseif($_GET['org']=='ysh'){//已签收待评价
			$nums=$db->get_one('select count(*) as num from `web_order` where userid='.$_userid.'  and status=4');
			$order=$db->getAll('select *from `web_order` where userid='.$_userid.'  and
			status=4 order by updatetime desc,addtime desc limit '.$start.','.$offset,"id");
		}elseif($_GET['org']=='ytk'){//退款
			$nums=$db->get_one('select count(*) as num from `web_order` where userid='.$_userid.'  and status=5');
			$order=$db->getAll('select *from `web_order` where userid='.$_userid.'  and
			 status=5 order by updatetime desc,addtime desc limit '.$start.','.$offset,"id");
		}else{
			$nums=$db->get_one('select count(*) as num from `web_order` where userid='.$_userid.'  and status>0');
			$order=$db->getAll('select * from `web_order` where userid='.$_userid.' and status>0 order by updatetime desc,addtime desc limit '.$start.','.$offset,"id");
		}
		...
		if($ismobile||$_GET['t']==11){
			include T('show/m/','order');
		}else{
			include T('admin','order');
		}
		break;
	case 'kd':
		$kd_arr=array(
