...
function add_o(){
	...
	var spec=$('#opt_new').val();
	var pos=$('#pos_new').val();
	$.ajax({
		type:'post',
		url:'/?m=admin&a=cate&c=savespec',
		data:{'title':title,'spec':spec,'pos':pos,'cateid':<?=$id?>},
		success:function(r){
			if(r=='ok'){
				alert('发布成功');
			}else{
...
function save(id){
	...
	var spec=$('#opt_'+id).val();
	var pos=$('#pos_'+id).val();
	$.ajax({
		type:'post',
		url:'/?m=admin&a=cate&c=savespec',
		data:{'title':title,'spec':spec,'pos':pos,'id':id,'cateid':<?=$id?>},
		success:function(r){
			if(r=='ok'){
				alert('保存成功');
			}else{
