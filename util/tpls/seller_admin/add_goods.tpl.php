...
layui.use('form', function(){
	...
	form.on('select(shop_pid)', function(data){
		var shop_pid=data.value;
		$.ajax({
			type:'post',
			url:'/home/ajaxshop.html',
			data:{'id':shop_pid},
			success:function(data){
				$('#ssid').html(data);
				form.render('select');
...
<div id="main" class="cont">
	...
	<div id="right">
		...
		<div class="r_main">
			<form method="post" action="index.php" id="cate_a_from" class="layui-form">
					...
					<div class="layui-form-item">
						...
						<div class="layui-input-inline">
							<select name="shop_pid" lay-filter="shop_pid">
								<option value="0" selected="selected">请选择分类</option>
							<?php foreach($shop_parent as $k=>$v) {?>
								<option value="<?=$v['id']?>"><?=$v['name']?></option>
							<?php }?>
							</select>
						</div>
					...
					<div class="layui-form-item">
						...
						<div class="layui-input-block">
							<input type="text" name="title" id="chk_title" value="<?=$goods['title']?>" class="layui-input" placeholder="请填写商品标题"/>
						</div>
					</div>
					<div class="layui-form-item">
						<label class="layui-form-label">商品副标题</label>
						<div class="layui-input-block">
							<input type="text" name="subtitle" lay-verify="subtitle" value="<?=$goods['subtitle']?>" autocomplete="off" placeholder="请填写商品副标题" class="layui-input">
						</div>
					</div>
					<div class="layui-form-item">
					...
					<div class="layui-form-item">
						<label class="layui-form-label">分销商品</label>
						<div class="layui-input-block">
							<input type="checkbox" <?php if($goods['is_fx']==1){?>checked<?php }?> name="is_fx" lay-skin="switch">
						</div>
					</div>
					<div class="layui-form-item">
						<label class="layui-form-label">分销佣金</label>
						<div class="layui-input-inline">
							<input type="text" name="fx_fee" value="<?=$goods['fx_fee']?>" lay-verify="fx_fee" autocomplete="off" class="layui-input">
						</div>
						<div class="layui-form-mid layui-word-aux">该商品的分销佣金，仅分销商品可用</div>
					</div>
					<div class="layui-form-item">
					...
					<div class="layui-form-item">
						...
						<div class="layui-form-mid layui-word-aux">必填项，单位千克/公斤，精确到小数点后两位,用于计算运费</div>
					</div>
					<div class="layui-form-item">
						...
						<div class="layui-form-mid layui-word-aux">必填项，单位立方米，精确到小数点后两位,用于计算运费</div>
					</div>
		...
		<?php
			$flvs=$db->getAll('select * from `web_shop_flv` where `uid`='.$_userid);
			if(!empty($flvs)){
				foreach($flvs as $flv) {?>
								<option value="<?=$flv['id']?>"<?=$flv['id']==$goods['flvid']?' selected':''?>><?=$fee_tp[$flv['type']]['tit']?>_<?=$flv['title']?></option>
				<?php }}?>
							</select><br/>
							<a href="/?m=admin&a=trans" class="layui-btn layui-btn-small">管理运费模板</a> 
						</div>
					</div>
					<div class="layui-form-item">
					...
					<div class="layui-form-item">
						...
						<div class="layui-input-block">
							<h4 style="height:30px;line-height:30px;overflow:hidden">提示：上传2-5张图片，jpg或者jpeg格式，单张图片不超过2MB，建议尺寸800X800像素，上传成功后，可以通过拖动图片来改变排序!</h4>				
							<div id="thumbnails">
...
function show_org(pic){
	show_area('/index.php?m=admin&a=showphoto&picid='+pic,'查看原图','720px',2);
}
var swfu;
