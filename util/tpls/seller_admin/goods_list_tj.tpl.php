...
<div id="main" class="cont">
	...
	<div id="right">
		<div class="r_tit">推荐商品</div>
		<p>共有推荐的商品 <span style="color:#78a001"><?=$nums['num']?></span> 个</p>
...
<?php foreach($list as $k=>$v) {?>
			<div class="li_good">
				...
				<div class="s5 fl">
					<p><a onclick="canceltj(<?=$v['id']?>)" class="layui-btn layui-btn-small">取消推荐</a></p>
				</div>
			</div>
<?php }?>
		</div>
...
function canceltj(id){
	if(confirm("确定要取消推荐该商品吗?")){ 
		$.ajax({
			type:'post',
			url:'/?m=admin&a=goods&c=canceltj',
			data:{'id':id},
			success:function(r){
				if(r=='ok'){
					layer.alert('商品取消推荐成功');
				}else{
					layer.alert(r);
				}
